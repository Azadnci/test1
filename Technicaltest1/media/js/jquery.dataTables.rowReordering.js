...
(function ($) {
   ...
   $.fn.rowReordering = function (options) {
      ...
      function fnGetState(oTable, sSelector, tr) {
         //var tr = $("#" + id, oTable);
      // ### END ###
         var iCurrentPosition = parseInt(oTable.fnGetData(tr[0], properties.iIndexColumn), 6);
         var iNewPosition = -1; // fnGetStartPosition(sSelector);
         var sDirection;
         var trPrevious = tr.prev(sSelector);
         if (trPrevious.length > 0) {
            iNewPosition = parseInt(oTable.fnGetData(trPrevious[0], properties.iIndexColumn), 6);
            ...
            if (trNext.length > 0) {
               iNewPosition = parseInt(oTable.fnGetData(trNext[0], properties.iIndexColumn), 6);
